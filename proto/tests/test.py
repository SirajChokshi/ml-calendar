...
for week in scraper.scrape().weeks:
    for day in week.days:
        event_sets.append(day.events)
...
for event_set in event_sets:
    for i in range(1, len(event_set) - 1):
        data_dict['start'].append(event_set[i - 1][0])
        data_dict['val'].append(event_set[i][0])
        data_dict['end'].append(event_set[i + 1][0] - (event_set[i][1] - event_set[i][0]))
